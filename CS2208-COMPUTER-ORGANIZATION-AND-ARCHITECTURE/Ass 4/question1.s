		...
		ENTRY
Start	ADR r0, UPC4 ;register r0 to point to the string
...
Loop      CMP r2, #0 ;checks to see if the sum is 0 yet or at all
          BEQ ValidExit ;if it is equal to 0, it is a multiple of 10, so jump to ValidExit
...
ValidExit MOV r0, #1 ;move number 1 to r0 and exit
Endless   B Endless ;this ends the program after a valid exit
InvExit   MOV r0, #2 ;move number 2 in r0 and exit
Endless2  B Endless2 ;this ends the program after an invalid exit
